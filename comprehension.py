...
* All "non-characters" (e.g. punctuation, numbers etc.) are currently omitted from the output.
...
def program(text, comprehension_as_ratio, language="NO"):
    ...
    word_dict = dict(zip(word_list, word_list)) # Make dictionary with unique words as keys, and same unique words as values
    ...
    if language == "CN": #If Chinese we omit white spaces from the final result
...
# TODO: Consider whether or not to use a parser for Chinese to separate words...
